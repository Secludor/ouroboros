...
class TestInterviewDoneStreakAndShortfall:
    ...
    async def test_explicit_done_non_qualifying_resets_stale_streak(self) -> None:
        """Pinned: explicit ``done`` with a weak rescore MUST reset stale streak.

        Design-note regression for PR #428 (8765c43): the two-signal
        completion contract becomes stateful in surprising ways if some
        paths invalidate stale streaks and others preserve them. A
        streak of 1 from an earlier qualifying signal must be cleared
        the moment the user's next explicit ``done`` rescores to a
        non-qualifying ambiguity — otherwise a single later qualifying
        confirmation would incorrectly finalize the interview from a
        poisoned baseline.
        """
        handler = InterviewHandler(llm_adapter=MagicMock())
        handler._emit_event = AsyncMock()
        state = InterviewState(
            interview_id="sess-428-stale",
            completion_candidate_streak=1,  # stale from an earlier signal
            ambiguity_score=None,
            ambiguity_breakdown=None,
            rounds=[
                InterviewRound(
        ...
        with (
            ...
            result = await handler.handle({"session_id": "sess-428-stale", "answer": "done"})

        assert result.is_ok
        assert state.status == InterviewStatus.IN_PROGRESS
        assert state.completion_candidate_streak == 0, (
            "Stale streak must be cleared when explicit 'done' rescores to a "
            "non-qualifying ambiguity — preserving it would let the next single "
            "qualifying signal finalize the interview from a poisoned baseline."
        )
        mock_engine.complete_interview.assert_not_called()

    async def test_shortfall_persist_failure_surfaces_error(self) -> None:
        """Pinned: shortfall persist is part of the correctness contract.

        Design-note regression for PR #428 (3c2531d): the shortfall
        branch persists the incremented streak, but that write is now
        load-bearing — a failed persist means the user's next ``done``
        advances from a wrong baseline. A best-effort write that
        silently swallows the failure must not return the "almost
        there" success message. The returned state must reflect the
        pre-advance streak (no in-memory lie) and the response must be
        an error.
        """
        from ouroboros.core.errors import ValidationError as _VE

        handler = InterviewHandler()
        handler._emit_event = AsyncMock()
        state = InterviewState(
            interview_id="sess-428-persist",
            completion_candidate_streak=0,
            ambiguity_score=0.14,
            ambiguity_breakdown={
                "goal_clarity": {
        ...
        save_failure = Result.err(
            _VE(
                "disk full",
                details={"interview_id": "sess-428-persist"},
            )
        )
        mock_engine = MagicMock()
        mock_engine.load_state = AsyncMock(return_value=Result.ok(state))
        ...
        with patch(
            ...
            result = await handler.handle({"session_id": "sess-428-persist", "answer": "done"})

        assert result.is_err, (
            "Shortfall persist failure must surface as an error, not a "
            "best-effort swallowed write with the 'almost there' message."
        )
        assert result.error.tool_name == "ouroboros_interview"
        assert "persist" in str(result.error).lower()
        mock_engine.save_state.assert_called()
